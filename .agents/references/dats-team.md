...
- Git over SSH signs through the 1Password agent and waits for the user's approval. When the user is not watching, use HTTPS with the `glab` token instead: `git -c credential.helper= -c 'credential.helper=!glab auth git-credential' push https://gitlab.dats.tech/<group>/<project>.git HEAD:refs/heads/<branch>`. Fetch into `FETCH_HEAD` and set the tracking ref with `git update-ref refs/remotes/origin/<branch> FETCH_HEAD`; a `<url> src:refs/remotes/origin/<branch>` refspec has dropped existing tracking refs. Verify the remote head through `glab api .../repository/branches/<branch>`.
- A CI value defined in neither the project repository nor `datsteam/ops/deploy` (for example `E2E_PLAYWRIGHT_IMAGE`) lives in GitLab CI/CD settings and belongs to OPS. Locate where a value is defined before proposing a change; for an OPS-owned value, report the required change and its timing to the user and keep repositories unchanged.
- View pipeline: `glab mr view <id>`, `glab ci status`
- Long runs (builds, E2E, CI pipelines) start in the background with a monitor whose filter matches failures as well as completion. Read and report each failure as it appears, and keep the user informed between events; the user should never have to background a command or ask whether it is stuck.
