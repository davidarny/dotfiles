...
- Use the Hindsight MCP server and the `shared-memory` bank for durable memory across Claude, Codex, OpenCode, Pi, Hermes, and ChatGPT. Before each response, recall relevant memories and use them as historical context for the current conversation.
...
- If Hindsight is unavailable, continue the task and report the failed recall or retain when it affects the result. Never claim a memory was saved without a successful tool response.
